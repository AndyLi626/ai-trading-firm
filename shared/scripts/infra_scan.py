...
def main():
    ...
    proposals.extend(_arch_lock_check())
    ...
    print(json.dumps({"status": "ok", "proposals": len(proposals), "summary": summary,
                      "output": out_path}))


def _arch_lock_check():
    """Run arch_lock.py check and add drift items to proposals."""
    ...
    try:
        ...
        items = []
        for drift in d.get("drift", []):
            items.append({
                "type": "arch_drift",
                "severity": "medium",
                "description": f"Arch drift: {drift['key']} {drift['change']}",
                "proposed_fix": "Review; if intentional run arch_lock.py generate",
                "requires_human_review": False,
