...
def main():
    ...
    sources_ok = sum(1 for q in quotes.values() if q.get("last_price") is not None)
    sources_total = max(len(quotes), 1)
    confidence = round(sources_ok / sources_total, 4)
    ...
    output = {
        ...
        "as_of": generated_at,
        "source": "alpaca_iex+hyperliquid" if crypto_syms else "alpaca_iex",
    ...
    compat_path = os.path.join(FACTS_DIR, "market_facts.json")
