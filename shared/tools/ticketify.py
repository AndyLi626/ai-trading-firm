#!/usr/bin/env python3
"""Convert an operator discussion into a tracked Infra ticket."""
import json
import os
import sys
...
def ticketify(discussion: str, priority: str = "normal", acceptance: str = "") -> dict:
    ...
    if not acceptance:
        acceptance = "Write evidence to file and include it in the next manager delta."
    t = enqueue(
        message=f"{title}\n\n{discussion}\n\ncriteria: {acceptance}",
        sender="manager",
        priority=priority,
...
if __name__ == "__main__":
    ...
    acceptance = next(
        (
            sys.argv[i + 1]
            for i, arg in enumerate(sys.argv)
            if arg == "--acceptance" and i + 1 < len(sys.argv)
        ),
        "",
    )
